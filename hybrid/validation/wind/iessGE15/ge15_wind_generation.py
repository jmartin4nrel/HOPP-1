...
def process_status(years, overwrite=False):
    ...
    if (filename not in files) or overwrite:
        ...
        for file in files:
            ...
            if sec_df.index[0].year in years:
        ...
        hour_df.to_json(current_dir / filename)

    return filename


def get_yaw_mismatch(yaw_file, tenmin_wind_file, years, overwrite=False):

    # Set initial directory
    current_dir = Path(__file__).parent.absolute()
    filename = yaw_file[:-4]+' mismatch'
    files = os.listdir(current_dir)
    
    if (filename not in files) or overwrite:
    
        yaw_mat = np.loadtxt(current_dir/yaw_file)
        yaw_mat = np.where(yaw_mat>=0, yaw_mat, np.full(np.size(yaw_mat), np.nan))
        tenmin_wind_df = pd.read_json(current_dir/tenmin_wind_file)

...
def tune_iess(period_file):
    ...
    good_period_file = validation_dir / "hybrid" / "iessGEFS" / period_file
    ...
    yaw_file, tenmin_wind_file = wind_gen.get_yaw_mismatch(yaw_file, tenmin_wind_file, years)
    status_file = wind_dir / wind_gen.process_status(years, overwrite=False)

    use_dir = True
    use_status = True

    hybrid_plant, overshoots = tune_data(hybrid_plant, tuning_files, resource_files, good_period_file,
                                            yaw_file, status_file, years, use_status, use_dir)

    return hybrid_plant, overshoots
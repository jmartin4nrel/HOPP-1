...
for i, sim_time in enumerate(sim_times):
    ...
    # Adjust forecast
    new_solar_fp = save_solar_forecast_SAM(solar_dict, sim_time, solar_file)
    new_wind_fp = save_wind_forecast_SAM(wind_dict, sim_time, wind_file)
    ...
    time_fn = run_id+\
                '_{:02d}'.format(sim_time.month)+\
                '_{:02d}'.format(sim_time.day)+\
                '_{:02d}'.format(sim_time.hour)
    ...
    j = list(year_hours).index(sim_time)
    xmin = sim_time-pd.Timedelta(3,unit='D')
    xmax = sim_time+pd.Timedelta(7,unit='D')
    plt.clf()
    plt.subplot(5,1,1)
    ...
    sec_df = time_df.loc[sim_time:sim_time+pd.Timedelta('2H')].resample('1S').interpolate()
    ...
    sec_fn = 'sec_df_{}.csv'.format(run_id)
    if sec_fn not in os.listdir(results_dir):

...
def import_m5_wind(start_month='August 2012', end_month='August 2012'):
    ...
    # Save to .json
    tenmin_df.to_json(resource_path/(start_month+' to '+end_month+' M5 wind 10 min'))
...
def downsample_m5_wind(bin_resolution=4, start_month='August 2012', end_month='August 2012'):
    ...
    current_path = Path(__file__).parent.absolute()
    resource_path = current_path / '..' / '..' / 'examples' / 'resource_files'
    tenmin_df = pd.read_json(resource_path/(start_month+' to '+end_month+' M5 wind 10 min'))
    ...
    for year_idx, year_start in enumerate(years[:-1]):
        ...
        hour_df.to_json(resource_path/export_name)
    save_bins = pd.DataFrame(bin_mat, index=hour_bins, columns=tenmin_bins)
    save_bins.to_json(resource_path/(start_month+' to '+end_month+' M5 wind {:.0f} bins 1 hr 10 min'.format(bin_resolution)))
...
def refactor_power_curve_m5_wind(power_curve, rotor_diameter, air_density=1.225, bin_resolution=4, start_month='August 2012', end_month='August 2012'):
    ...
    refactored_power_curve.to_csv(resource_path/(power_curve[:-4]+'_Refactored_Hourly.csv'),index=False)